...
public class RedisCacheEngine implements CacheEngine, IConstants {
   private static Logger logger = Logger.getInstance(RedisCacheEngine.class);
   ...
   public void initialize(Properties props) {
      if (initialized) {
         throw new UnsupportedOperationException("Init method has been called before!");
      }
      if (props == null) {
      ...
      evaluators = new TreeMap<PropertyEvaluator, PropertyContext>(new PropertyEvaluatorComparator());
      evaluators.put(new DefaultPropertyEvaluator(), new DefaultPropertyContext(settings));
      evaluators.put(new RedisInstancePropertyEvaluator(), new RedisInstancePropertyContext(redisInfos));
   ...
   public void configure(String param, String value) {
      ...
      try {
         ...
         String msg = MessageFormat.format("Error occurred during apply the setting {0}={1}", param, value);
   ...
   public void configure(Map<String, String> settings) {
      ...
      for (Entry<String, String> e : settings.entrySet()) {
         ...
         try {
            ...
            String msg = MessageFormat.format("Error occurred during apply the setting {0}={1}, cause:{2}", key, value,
                  ex.getMessage());
            logger.log(LogLevel.ERROR, msg, ex);
         }
      }
   ...
   private void loadProperties(Properties props) {
      ...
      for (Entry<Object, Object> pair : props.entrySet()) {
         ...
         for (Entry<PropertyEvaluator, PropertyContext> entry : evaluators.entrySet()) {
            if (entry.getKey().qualify(key)) {
               try {
                  entry.getKey().process(key, value, entry.getValue());
               } catch (EvaluateException e) {
                  String msg = MessageFormat.format("Error Occured while proccessing line:{0},{1}={2}", line, key,
                        value);
   ...
   private void initConnection() {
      ...
      for (Entry<String, RedisInfo> entry : redisInfos.entrySet()) {
         ...
         JedisShardInfo shard = new JedisShardInfo(info.getHost(), info.getPort(), info.getTimeout(),
               Integer.valueOf(info.getWeight()));
   ...
   private void set(String key, Object value, int expire) {
      ...
      try {
         jedis = pool.getResource();
         byte[] binKey = SafeEncoder.encode(key);
         jedis.set(binKey, SerializeUtil.serialize(value));
         if (expire > 0) {
            jedis.expire(binKey, expire);
         }
         logger.log(LogLevel.WARN, expire + " stored in redis");
      } finally {
         if (jedis != null) {
            pool.returnResource(jedis);
         }
   ...
   private Object get(String realKey) {
      ShardedJedis jedis = null;
      try {
         jedis = pool.getResource();
         byte[] binKey = SafeEncoder.encode(realKey);
         logger.log(LogLevel.WARN, realKey + " retrived from redis");
   ...
   public void addToList(IApplicationKey appKey, ISectionKey secKey, String key, Object value, int index, int expire) {
      ...
      try {
         ...
         if (index == 0) {
            ...
            jedis.linsert(binKey, LIST_POSITION.BEFORE, pivot, SerializeUtil.serialize(value));
         }
         if (realExpire > 0) {
   ...
   public List<Object> getList(IApplicationKey appKey, ISectionKey secKey, String key, int begin, int end) {
      if (begin > end && end != -1) {
         throw new IllegalArgumentException("Wrong index of begin and end of the list");
      }
      ShardedJedis jedis = null;
      ...
      try {
         ...
         String realKey = RedisKeyUtil.getKey(appKey, secKey, key);
         byte[] binKey = SafeEncoder.encode(realKey);
         result = (List<Object>) SerializeUtil.unserialize(jedis.lrange(binKey, begin, end));
      } finally {
         if (jedis != null) {
            pool.returnResource(jedis);
         }
   ...
   public Set<Object> getSet(IApplicationKey appKey, ISectionKey secKey, String key) {
      ShardedJedis jedis = null;
      Set<Object> result = Collections.EMPTY_SET;
      try {
         jedis = pool.getResource();
         String realKey = RedisKeyUtil.getKey(appKey, secKey, key);
         byte[] binKey = SafeEncoder.encode(realKey);
         result = (Set<Object>) SerializeUtil.unserialize(jedis.smembers(binKey));
      } finally {
         if (jedis != null) {
            pool.returnResource(jedis);
         }
